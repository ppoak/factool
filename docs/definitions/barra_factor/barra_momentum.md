...
**相关数据：** quotes_day表中的close_post列（后复权收盘价）、st列（是否为ST，如为是则为True）、suspended列（是否停牌，停牌则为True）。对于停牌、ST的股票，因子值为np.nan，可以通过st和suspended数据作为mask过滤处可以计算的股票。
...
其中$ w_t$为t时刻对应的指数加权平均系数。

**说明：** 用于衡量剔除最近一个月的涨跌的长期动量。
