...
| 流通股本                 | `circulation_a` | A 股流通股数       | 需自行计算换手率     |
| 成交金额                 | `amount_post`   | 当日成交额（复权） | 可用于验证成交活跃度 |
...
2. **处理异常值：**
   ...
   - 若停牌（`suspended=True`），则该日换手设为 0。
3. **计算流程：**

   - 筛选对应时间窗口的日线数据；
   - 对每支股票（`code`）按周分组；
   - 按时间顺序计算 $RP_t$ 与 $CGO_t$；
   - 可将结果合并为 DataFrame 并存入因子库。
...
- **频率：** 本定义以周频为主；如需日频，可将 $V_t$ 替换为日换手并令 $T$ 为日数。

### 常见变体

- **滚动平滑**：对 \(CGO_t\) 进行短窗均值/中位数平滑以降噪。
- **行业/市值中性化**：回测前按行业、市值回归得到残差因子。
- **阈值截断**：对极端 \(CGO_t\) 做去极值（如 1%–99% winsorize）与标准化。
