...
class EvaluationFactor(BaseFactor):
    ...
    def get_mlev(self, date: str | pd.Timestamp) -> pd.Series:
        ...
        #优先股账面价值
        pe = ffin.read('equity_preferred_stock', start=rollback, stop=date).reindex(trading_days).ffill().loc[date].fillna(0)

        #长期负债账面价值
        ld = ffin.read('non_current_liabilities', start=rollback, stop=date).reindex(trading_days).ffill().loc[date].fillna(0)
    ...
    def get_dtoa(self, date: str | pd.Timestamp) -> pd.Series:
        ...
        #总资产账面价值
        ta = ffin.read('total_assets', start=rollback, stop=date).reindex(trading_days).ffill().loc[date].fillna(0)
