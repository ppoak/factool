...
class VolatileFactor(Factor):
    ...
    def get_daily_return_volatility(self, date: str) -> pd.Series:
        ...
        res = np.sqrt(((stock_ret - stock_ret.mean()) ** 2).sort_index().ewm(halflife=42, adjust=False).mean().sum())
        res.name = date
        return res
    ...
    def get_beta_return_residual(self, date: str) -> pd.Series:
        ...
        price = quotes_day.read("close", start=rollback, stop=date)
        _adj= quotes_day.read("adjfactor", start=rollback, stop=date)
        stock_ret = (price * _adj).pct_change(fill_method=None).tail(252).sort_index().ewm(halflife=63, adjust=False).mean()
        market_ret = index_quotes_day.read('close', code='000985.XSHG', start=rollback, stop=date).pct_change(fill_method=None).tail(252).sort_index().ewm(halflife=63, adjust=False).mean().loc[:,'000985.XSHG']
