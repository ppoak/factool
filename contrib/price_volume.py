...
class PriceVolumeDay(BaseFactor):

    def calc_compound_volume(self, time: pd.Timestamp | str) -> pd.Series:
        rollback = self.source.get_time(time, 21)[0]
        diff_price = (
            self.source.get_factor("close", begin=rollback, end=time).diff(1).iloc[-20:]
        )
        diff_volume = (
            self.source.get_factor("volume", begin=rollback, end=time)
            .diff(1)
            .iloc[-20:]
        )
        ...
        return pd.concat(
            [
                ...
                corr_neg_pos,
                corr_neg_neg,
                corr_pos_pos - corr_pos_neg - corr_neg_pos + corr_neg_neg,
            ],
            axis=1,
            keys=[
                "corr_pos_pos",
...
class PriceVolumeMin(BaseFactor):
    ...
    def calc_smart_money_ratio(self, date: pd.Timestamp) -> pd.DataFrame:
        ...
        res = ((retvol * price).sum() / retvol.sum()) / (
            (volume * price).sum() / volume.sum()
        )
        res.name = date
        return res
    ...
    def calc_trend_fund(self, date: str):
        ...
        volume_5d = quotes_min.read(
            "volume", begin=rollback, end=date + pd.Timedelta(days=1)
        )
    ...
    def get_5d_trend_fund_div_meanprice(self, date: str):
        ...
        volume_5d_90 = quotes_min.read("volume", start=rollback, stop=date).quantile(
            0.90
        )
    ...
    def calc_price_spread(self, date: str):
        ...
        for stock in distance_matrix.columns:
            nearest_stocks[stock] = (
                distance_matrix[stock].sort_values(ascending=True)[1:11].index.tolist()
            )
    ...
    def calc_minute_rsi(self, date: str):
        price = quotes_min.read(
            ...
            pivot="close",
            datetime__ge=date,
            datetime__le=date + pd.Timedelta(days=1),
        )
    ...
    def calc_20d_max_truerange(self, date: str):
        ...
        prev_close = (quotes_day.read("close", start=rollback, stop=date) * _adj).shift(
            1
        )
        ...
        res = np.maximum.reduce(
            [
                (high - low) / prev_close,
                abs(high - prev_close) / prev_close,
                abs(prev_close - low) / prev_close,
            ]
        )  # 每行最大值
    ...
    def calc_industry_co_20d_maxret(self, date: str) -> pd.Series:
        ...
        ind = industry_info.read(start=date, stop=date)[
            "first_industry_name"
        ].reset_index(level="date", drop=True)
        ind_ret = industry_returns.read("000300.XSHG", start=rollback, stop=date).tail(
            20
        )
    ...
    def calc_industry_co_20d_top5_cum_ret(self, date: str) -> pd.Series:
        ...
        res = max_ret.apply(
            lambda row: row[:-1].apply(
                lambda date: get_industry_return(date, row["industry"])
            ),
            axis=1,
        ).sum(axis=1)
    ...
    def calc_industry_co_20d_top5_cum_pricevolume_weighted(
        self, date: str
    ) -> pd.Series:
        rollback = quotes_day.get_trading_days_rollback(date, 20)
        price = quotes_day.read("close", start=rollback, stop=date)
    ...
    def calc_industry_co_20d_bottom15_cum_pricevolume_weighted(
        self, date: str
    ) -> pd.Series:
        rollback = quotes_day.get_trading_days_rollback(date, 20)
        price = quotes_day.read("close", start=rollback, stop=date)
        ...
        ret = (price * _adj).pct_change(fill_method=None).tail(20)
        volume = quotes_day.read("volume", start=rollback, stop=date).tail(20)
        smallest_price_volume = (
            (ret * volume).apply(lambda x: x.nsmallest(15).index, axis=0).T
        )
        ...
        res = smallest_price_volume.apply(
            lambda row: row[:-1].apply(
                lambda date: get_industry_return(date, row["industry"])
            ),
            axis=1,
    ...
    def calc_compound_industry_co_reverse_momemtum(self, date: str) -> pd.Series:
        vicm = self.read(
            "industry_co_20d_top5_cum_pricevolume_weighted", start=date, stop=date
        ).squeeze()
        vicr = self.read(
            "industry_co_20d_bottom15_cum_pricevolume_weighted", start=date, stop=date
        ).squeeze()
        res = vicm - vicr
        res.index.name = "order_book_id"
        res.name = date
    ...
    def calc_intraday_ret_str_000985(self, date: str) -> pd.Series:
        ...
        price = quotes_min.read(
            "close", code=universe, start=date, stop=date + pd.Timedelta(days=1)
        )
    ...
    def calc_10minute_str_000985(self, date: str) -> pd.Series:
        ...
        ret = price.pct_change(periods=5, fill_method=None)
        res = (ret.sub(ret.mean(axis=1), axis=0)).abs() / (
            ret.abs().add(np.abs(ret.mean(axis=1)), axis=0) + 0.01
        )
        res = res.mean()
        res.name = date
        return res
    ...
    def calc_stv(self, date: str) -> pd.Series:
        ...
        res = (turnover - turnover.mean()).abs() / (
            turnover.abs() + np.abs(turnover.mean()) + 0.1
        )
        res.name = date
        return res
    ...
    def get_doc(self, date: str) -> pd.DataFrame:
        df = quotes_min.read(
            ["volume", "close"], start=date, stop=date + pd.Timedelta(days=1)
        )
        ...
        def _doc(ret, volume):
            group = pd.qcut(
                ret.rank(method="first", ascending=True), q=20, labels=False
            )
